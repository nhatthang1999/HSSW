...
import { editMedicalRecordsForStaff } from "../../reducers/MedicalRecordManagement";
import { useDispatch } from "react-redux";

const Wrapper = styled.div``;
const EditMedicalRecordModal = (props) => {
  ...
  const handleOk = async () => {
    const response = await dispatch(
      editMedicalRecordsForStaff({
        medical_record_id: props.item.medical_record_id,
    ...
    setTimeout(async () => {
      if (response.status === 200)
        await notification.success({
          message: "Sửa hồ sơ bệnh án thành công!",
          duration: 2.5,
        });
      else
        await notification.error({
          message: "Sửa hồ sơ bệnh án không thành công!",
    ...
    >
      <Wrapper>
        <Form layout="vertical" method="post" form={form} onFinish={handleOk}>
          ...
          >
            <Input name="customer_code" onChange={handleOnChange} disabled />
          </Form.Item>
          <Form.Item
            label="Tên bệnh"
            name="disease_name"
